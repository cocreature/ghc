...
pprLlvmCmmDecl :: LabelMap DebugBlock -> MetaId -> LlvmCmmDecl -> LlvmM (SDoc, [LlvmVar])
pprLlvmCmmDecl _ _ (CmmData _ lmdata)
  = return (vcat $ map pprLlvmData lmdata, [])

pprLlvmCmmDecl debug_map cuMeta (CmmProc (label, mb_info) entry_lbl live (ListGraph blks))
